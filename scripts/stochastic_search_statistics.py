...
class Stochastic_Search_Statistics():
    ...
    def create_dataframe_paper(self, folder, subject = 1):
        ...
        df = pd.DataFrame(columns = ['ID', 'Regen Number', 'Sample Indicator', 'SSIM', 'Pixel Correlation', 'CLIP Cosine', 
        ...
        for i in tqdm(idx, desc="creating dataframe rows"):
            ...
            for filename in os.listdir(path): 
                ...
                if("Ground Truth.png" not in filename):
                    ...
                    _, _, clip_cosine = self.calculate_clip_similarity_papaer(folder, i, regen_count, subject = subject)
                    ...
                    row = pd.DataFrame({'ID' : str(i), 'Regen Number' : str(regen_count), 'Sample Indicator' : "1",
                                        ...
                                        'CLIP Cosine' :  str(round(clip_cosine, 10))}, index=[df_row_num])
        ...
        print(df.shape)
        print(df)
        df.to_csv(dataframe_path + "statistics_df_cortical_convolution_" + str(len(idx)) +  ".csv")
    ...
    def create_dataframe(self, experiment_name, subject = 1):
     ...
     971, 974, 976, 977, 978, 979, 980, 981]
        ...
        print(df.shape)
        print(df)
        df.to_csv(directory_path + "statistics_df_brain_correlation" + str(len(idx)) +  ".csv")
...
def main():
    ...
    #SCS.create_dataframe("Final Run: SCS UC LD 6:100:4 Dual Guided clip_iter", subject=1)
    
    SCS.create_dataframe_paper("Cortical Convolutions", subject=1)
