...
class Masker():
    def __init__(self, 
        ...
        # Initialize the data
        _, _, self.x_vox, self.x_thresh, self.x_test, _, _, self.y_vox, self.y_thresh, self.y_test, _ = load_nsd_vs(vector="c_img_0", loader=False)
    ...
    def mask_voxels(self, threshold, voxels):
        ...
        mask = torch.load("/export/raid1/home/kneel027/Second-Sight/masks/" + self.encoderModel + "/" + str(threshold) + ".pt")
        masked_x = torch.zeros((len(voxels), torch.count_nonzero(mask)))
        # for i in tqdm(range(len(voxels)), desc=(self.vector + " masking")):
        masked_x = voxels[:, mask]
        return masked_x
           
    def get_percentile(self, threshold):
        print("finding percentile for " + str(threshold))
        masked_threshold_x = self.mask_voxels(threshold, self.x_thresh)
        masked_threshold_encoded_x = self.mask_voxels(threshold, self.x_thresh_encoded)
        ...
        for i in tqdm(range(masked_threshold_x.shape[0]), desc="scanning library for threshold " + str(threshold)):
            ...
            for batch in tqdm(range(124), desc="batching sample"):
                x_preds = torch.load(latent_path + "/cc3m_batches/" + str(batch) + ".pt")
                x_preds = self.mask_voxels(threshold, x_preds)
                x_preds_t = x_preds.moveaxis(0, 1).to(self.device)
                
                # Pearson correlation
                scores[22735*batch:22735*batch+22735] = PeC(xDup, x_preds_t).detach()
            scores[-1] = PeC2(masked_threshold_x[i].to(self.device), masked_threshold_encoded_x[i].to(self.device)).detach()
