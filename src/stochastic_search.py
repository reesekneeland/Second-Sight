...
class StochasticSearch():
    ...
    def zSearch(self, beta, c_i, init_img=None, refine_z=True, refine_clip=True, n=10, max_iter=10, n_branches=1, absolute_weighting=False):
        ...
        for cur_iter in tqdm(range(max_iter), desc="search iterations"):
            ...
            combined_preds = torch.zeros((len(self.EncType), len(samples), beta.shape[1]))
            # if(self.ae):
            #     autoencoded_betas = torch.zeros((len(self.EncType), beta.shape[1]))
            print("COMBINED PREDS SHAPE: {}".format(combined_preds.shape))
            for c, mType in enumerate(self.EncType):
                if mType == "images" or mType == "alexnet_encoder_sub1":
                    combined_preds[c] = self.EncModels[c].predict(samples)
                elif mType == "c_img_uc":
                    combined_preds[c] = self.EncModels[c].predict(torch.stack(sample_clips)[:,0,:])
            combined_preds = combined_preds.to(self.device)
            if absolute_weighting:
