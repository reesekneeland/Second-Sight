...
from src.numpy_utility import iterate_range, make_gaussian_mass_stack, make_gaussian_mass, select_along_axis
...
def learn_params_ridge_regression(data, voxels, _fmaps_fn, models, lambdas, aperture=1.0, _nonlinearity=None, zscore=False, sample_batch_size=100, voxel_batch_size=100, holdout_size=100, shuffle=True, add_bias=False):
    ...
    with torch.no_grad():
        for m,(x,y,sigma) in enumerate(models):
            ...
            _pfs = [_to_torch(make_gaussian_mass(x, y, sigma, n_pix, size=aperture, dtype=dtype)[2], device=device) for n_pix in fmaps_rez]
            ...
            for rv,lv in iterate_range(0, nv, voxel_batch_size):
                ...
                if np.sum(imp)>0:
                    ...
                    if zscore:
                        ...
                        mst_std[arv]  = msts
                    best_w_params[arv,:] = select_along_axis(betas[:,:,imp], li, run_axis=2, choice_axis=0).T
