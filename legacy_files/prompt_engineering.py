...
def main():
    ...
    reconstructNImages(experiment_title="MLP Params prompt engineering 2", idx=[i for i in range(21)])
...
def reconstructNImages(experiment_title, idx):
    ...
    strength_z = 0
    R = Reconstructor()
    prompt1 = "high quality"
    prompt2 = "realistic"
    prompt3 = "4k"
    engineered_prompt1 = R.get_txt_c(prompt1).reshape((768,))
    engineered_prompt2 = R.get_txt_c(prompt2).reshape((768,))
    engineered_prompt3 = R.get_txt_c(prompt3).reshape((768,))
    for i in tqdm(idx, desc="Generating reconstructions"):
        ...
        c_engineered1 = format_clip(torch.stack([outputs_c_i[i], outputs_c_t[i], 0.5*engineered_prompt1]))
        c_engineered2 = format_clip(torch.stack([outputs_c_i[i], outputs_c_t[i], 0.5*engineered_prompt2]))
        c_engineered3 = format_clip(torch.stack([outputs_c_i[i], outputs_c_t[i], 0.5*engineered_prompt3]))
        ...
        # # Make the engineered reconstrution images. 
        engineered1 = R.reconstruct(c=c_engineered1, strength=strength_c)
        engineered2 = R.reconstruct(c=c_engineered1, strength=strength_c)
        engineered3 = R.reconstruct(c=c_engineered2, strength=strength_c)
        engineered4 = R.reconstruct(c=c_engineered2, strength=strength_c)
        engineered5 = R.reconstruct(c=c_engineered3, strength=strength_c)
        engineered6 = R.reconstruct(c=c_engineered3, strength=strength_c)
        ...
        rows = 4
        columns = 2
        images = [ground_truth, reconstructed_output_c, engineered1, engineered2, engineered3, engineered4, engineered5, engineered6]
        captions = ["Ground Truth", "c_combined", "prompt1", "prompt1", "prompt2", "prompt2"," prompt3", "prompt3",]
